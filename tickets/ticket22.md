...
Статические поля имеют static storage duration (см. [билет 15]()), поэтому внутри единицы трансляции гарантируется, что инициализация в том порядке, как она записана в файле (кроме некоторых `const`! так как они инициализируются через *const initialization* обычно сразу на этапе компиляции).

Порядок удаления гарантируется, что обратен порядку инициализации.

Больше про это можно прочитать в [билете 33]() про static initialization fiasco.
