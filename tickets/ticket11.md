...
* ### Взаимная рекурсия для: функций, классов, методов внутри одного класса, методов между классами (A::foo() возвращает B и наоборот).
    ...
    * Кусочек кусочка  
      ...
      Кароче статик и дружбан позволяют объявлять и определять функции вне структурки, единственное отличие, что статик
      лежит внутри класса, с точки зрения имени, те каждый раз писать Foo::bar().
        ```c++
...
Слов нет, одни эмоции. Кста qualified - это найти при помощи unqualified нужное место, а потом просто посмотреть внутрь.
qualified - он поднимается по уровнням наверх, когда найдет - остановится.(а лол это 13 билет)
...
коллизия имен. Прикольны пример ```04-210923/01-functions/02-lcm```. Там мы делаем лонговый лцм, но вызываем от интов,
