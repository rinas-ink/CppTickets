# Ticket 31 - Программы из нескольких файлов:
...
**`Note:`** Если не делать `include-guards`, то возможна ситуация: мы имеем структуру `Foo` в `"header1.h"` файле, далее мы написали `#include "header1.h"` в каком-нибудь `"header2.h"` файле, а затем в `main.cpp` сделали `#include "header1.h"` и `#include "header2.h"`, так мы получили `multiple definition of struct Foo`, чтобы такого не произошло делают следующее:
...
3. Опасность неявного включения, концепция include what you use, примеры из стандартной библиотеки вашего компилятора (`exercises/01-210906/07-missing-include`):
	...
	4. `ATTENTION! NO INFO FOUND:` Запрет на реализацию функций: *здесь непонятно, что хотят услышать*
