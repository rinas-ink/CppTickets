...
* `push_back(val)` - сначала инициализирует новый элемент как копию `val`, а затем мувает в него
  значение `val` и кладёт в вектор. Нужен готовый элемент копируемого типа.
* `emplace_back(args)` - вызывает конструктор типа, который лежит в векторе, передавая ему
  аргументы `args`. Конструктор может быть любым, копируемость не обязательна, работает быстрее,
  т.к. не создает временного объекта.
*
...
* `map` умеет сортировать элементы в соответствии со специальной булевой функцией `compare`, которая
  может передаваться в качестве опционального третьего аргумента. Стандартный компаратор
  -- `std::less` - пытается сравнивать ключи на `<`, результат сортировки - ключи упорядочены по
  возрастанию.
