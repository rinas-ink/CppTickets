...
delete b; // but base does not
```

* Здесь мы пытаемся вызвать деструктор от `Base`, которого нет, из-за этого UB. Чтобы этого
  избежать, у `Base` должен быть виртуальный деструктор.
...
* Как защититься: не передавать по значению, а вместо этого хранить ссылки/умные указатели. ***._.
  хз, что тут нужно***
