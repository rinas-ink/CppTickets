...
* Подробнее:
  * `int:` 
    * По стандарту, `short` - минимум 2 байта, `int` - минимум 2 байта, `long` - минимум 4 байта, `long long` -
      минимум 8 байт.
    * В документации `GCC` единственное отличие - `int` занимает 4 байта вместо 2.
  * `char:`
    * `UTF-8` - 8 бит (1 байт), `UTF-16` - 16 бит (2 байта) и `UTF-32` - 32 бита (4 байта). 
    * В `GCC` всё совпадает.
